...
SoftwareSerial Ex(pin_Ex_rx, pin_Ex_tx);
Servo channel_1;
Servo channel_2;
Servo channel_3;
Servo channel_4;
...
void reset_the_Data(){
    ...
    received_data.E = 127;
    received_data.A = 127;
    received_data.M = 0;  // atti mode                  -> need check
}
...
void loop() {
    ...
    ch6_value = map(received_data.M, 0,   1, 1000, 2000);//     -> only for internal logic
    ...
    if(Ex.available()){
        if(Ex.read() == '!'){
            count = 0;
            while(1){
                temp_read = (char)(Ex.read());
                if(temp_read == '*') { break; }
                temp_lat[count] = temp_read;
                count++;
            }
            ...
            while(1){
                temp_read = (char)(Ex.read());
                if(temp_read == '#') { break; }
                temp_alt[count] = temp_read;
                count++;
            }
            cur_lat = atof(temp_lat);
            cur_lng = atof(temp_lng);
            cur_alt = atof(temp_alt);
        }
    }
}